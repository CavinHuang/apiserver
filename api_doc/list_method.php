                    ...
                    if (!empty($methods)) {
                      ...
                      $config_data[$class_name] = [];
                        foreach ($methods as $key => $item) {
                        	$config_data[$class_name][$item['service']] =  $item['title'];
                            $NO = $num++;
                            echo '<tr>';
                            echo '<td>'.$NO.'</td>';
                        ...
                        $config_path = API_DOC_PATH.'/config/methods.php';
                        
                         if(!file_exists($config_path)){
                           $config_str .= var_export($config_data, true) .'?>';
                         }else{
                           $old_config = include_once($config_path);
                           $old_config[$class_name] = $config_data[$class_name];
                           $config_str .= var_export($old_config, true) .'?>';
                         }
